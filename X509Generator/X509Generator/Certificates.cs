...
namespace X509Generator
{
    ...
    {
        public static X509Certificate2 GenerateCertificate(string subject, string organisation,int validTime, string country = "DE")
        {
            ...
            certificateGenerator.SetIssuerDN(new X509Name($"C={country}, O={organisation}, CN={subject}"));
            certificateGenerator.SetSubjectDN(new X509Name($"C={country}, O={organisation}, CN={subject}"));
            certificateGenerator.SetNotBefore(DateTime.UtcNow.Date.AddDays(-1));
            certificateGenerator.SetNotAfter(DateTime.UtcNow.Date.AddYears(validTime));
        ...
        static private X509Certificate2 LoadCertificate(string subject)
        {
        ...
        static private void SaveCertificate(X509Certificate2 certificate)
        {
            var userStore = new X509Store(StoreName.My, StoreLocation.CurrentUser);
